...
int main(int argc, char *argv[]) {
  const String keys =
      ...
      "{d              |           | actual distance between top-left and "
  ...
  parser.about(
      ...
      "Usage: camera_calibration [configuration_file -- default "
      "./default.xml]\n"
  ...
  // FileStorage fout("settings.yml", FileStorage::WRITE); // write config as
  // YAML fout << "Settings" << s;
  ...
  for (;;) {
    ...
    if (found) // If done with success,
    {
      ...
      if (s.calibrationPattern == Settings::CHESSBOARD) {
        ...
        cornerSubPix(
            viewGray, pointBuf, Size(winSize, winSize), Size(-1, -1),
  ...
  if (s.inputType == Settings::IMAGE_LIST && s.showUndistorsed) {
    ...
    if (s.useFisheye) {
      ...
      fisheye::estimateNewCameraMatrixForUndistortRectify(
          cameraMatrix, distCoeffs, imageSize, Matx33d::eye(), newCamMat, 1);
      fisheye::initUndistortRectifyMap(cameraMatrix, distCoeffs, Matx33d::eye(),
                                       newCamMat, imageSize, CV_16SC2, map1,
                                       map2);
    } else {
...
static bool runCalibration(Settings &s, Size &imageSize, Mat &cameraMatrix,
                           ...
                           vector<Point3f> &newObjPoints, float grid_width,
                           bool release_object) {
  //! [fixed_aspect]
...
static void saveCameraParams(Settings &s, Size &imageSize, Mat &cameraMatrix,
  ...
  if (s.writeExtrinsics && !rvecs.empty() && !tvecs.empty()) {
    ...
    fs.writeComment("a set of 6-tuples (rotation vector + translation vector) "
                    "for each view");
