...
PipelineExitStatus
CalibPipeline::start(std::vector<cv::Ptr<FrameProcessor>> processors) {
  ...
  mImageSize = cv::Size((int)mCapture.get(cv::CAP_PROP_FRAME_WIDTH),
                        (int)mCapture.get(cv::CAP_PROP_FRAME_HEIGHT));
  ...
  while (mCapture.grab()) {
    ...
    for (std::vector<cv::Ptr<FrameProcessor>>::iterator it = processors.begin();
...
cv::Size CalibPipeline::getImageSize() const { return mImageSize; }
